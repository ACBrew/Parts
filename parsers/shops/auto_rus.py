    ...
    def __init__(self, search=''):
        ...
        cookies_dict = [
            {'domain': key.domain,
             ...
             'path': key.path,
             'value': key.value}
            for key in Cookie.objects.filter(title='autorus').all()
        ]
    ...
    def parsing_block(self, block):
        ...
        if delivery_date_row:
            ...
            if 'На складе' in delivery_date_row:
                ...
                hrs = int(re.search(r'\d*(?= час)', delivery_date_row).group())
                delivery_date = hrs // 24 + 1 if hrs % 24 > 0 else hrs//24
            elif 'дн' in delivery_date_row:
                delivery_date = int(re.search(r'\d*(?= дн)', delivery_date_row).group())
        else:
