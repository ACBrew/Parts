...
class STParts():
    ...
    def __init__(self, search=''):
        ...
        cookies_dict = [
            {'domain': key.domain,
             ...
             'path': key.path,
             'value': key.value}
            for key in Cookie.objects.filter(title='stparts.ru').all()
        ]
    ...
    def check_of_response(self, text):
        ...
        attention = soup.find('div', class_='fr-alert fr-alert-warning')
        if not attention:
            # проверка наличия дополнительях ссылок
            marker = soup.find('div', id='searchResultsHtml')
            if marker:
                ...
                list_of_link = []
                rows = soup.find_all('tr', class_='startSearching')
                for row in rows:
                    link = f"https://stparts.ru{row['data-link']}"
                    if not link:
                        continue
    ...
    def parse_page(self, container):
        ...
        for page in container:
            ...
            if button:
                ...
                params = {
                    'url': 'https://stparts.ru/searchResults',
                    'session': self.session,
                    'title': 'stparts.parse_page',
                    'params': {
    ...
    def parsing_block(self, block):
        ...
        brand_name = brand_number_img_data.get('data-brand', None)
        if not brand_name:
            logger.error('no brand name')

        number = brand_number_img_data.get('data-code', None)
        if not number:
            logger.error('no finis')

        url = f"https://stparts.ru/search/{brand_name}/{number}"
        if not url:
        ...
        delivery_date = block.find('td', class_='resultDeadline').text.split()
        if delivery_date:
            delivery_date = delivery_date[2]
        else:
        ...
        image_src = f"https:{brand_number_img_data['src']}"
        image_url = 'no url'
        if image_src:
    ...
    def run(self):
        text = self.get_page(url=f'https://stparts.ru/search?pcode={self.search}')
        if text:
